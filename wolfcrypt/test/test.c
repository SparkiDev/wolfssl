...
static int dh_generate_test(WC_RNG *rng)
{
    ...
    if (ret != BAD_FUNC_ARG) {
        ERROR_OUT(-7797, exit_gen_test);
    }
    ret = wc_DhSetKey(&smallKey, NULL, sizeof(p), g, sizeof(g));
    if (ret != BAD_FUNC_ARG) {
        ERROR_OUT(-7798, exit_gen_test);
    }
...
#ifdef WOLFCRYPT_HAVE_ECCSI
static int eccsi_api_test(WC_RNG* rng, mp_int* ssk, ecc_point* pvt)
{
    int ret;
    EccsiKey key;
    byte id[1] = { 0x00 };
    ...
    ret = wc_MakeEccsiPair(NULL, NULL, WC_SHA256, NULL, 1, NULL, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10031;
    ret = wc_MakeEccsiPair(&key, rng, WC_SHA256, id, 1, ssk, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10032;
    ret = wc_MakeEccsiPair(&key, rng, WC_SHA256, id, 1, NULL, pvt);
    if (ret != BAD_FUNC_ARG)
        return -10033;
    ret = wc_MakeEccsiPair(&key, rng, WC_SHA256, NULL, 1, ssk, pvt);
    if (ret != BAD_FUNC_ARG)
        return -10034;
    ret = wc_MakeEccsiPair(&key, NULL, WC_SHA256, id, 1, ssk, pvt);
    if (ret != BAD_FUNC_ARG)
        return -10035;
    ret = wc_MakeEccsiPair(NULL, rng, WC_SHA256, id, 1, ssk, pvt);
    if (ret != BAD_FUNC_ARG)
        return -10036;
    /* No key set */
    ret = wc_MakeEccsiPair(&key, rng, WC_SHA256, id, 1, ssk, pvt);
    if (ret != BAD_STATE_E)
        return -10037;

    ret = wc_ValidateEccsiPair(NULL, WC_SHA256, NULL, 1, NULL, NULL, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10038;
    ret = wc_ValidateEccsiPair(&key, WC_SHA256, id, 1, ssk, pvt, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10039;
    ret = wc_ValidateEccsiPair(&key, WC_SHA256, id, 1, ssk, NULL, &valid);
    if (ret != BAD_FUNC_ARG)
        return -10040;
    ret = wc_ValidateEccsiPair(&key, WC_SHA256, id, 1, NULL, pvt, &valid);
    if (ret != BAD_FUNC_ARG)
        return -10041;
    ret = wc_ValidateEccsiPair(&key, WC_SHA256, NULL, 1, ssk, pvt, &valid);
    if (ret != BAD_FUNC_ARG)
        return -10042;
    ret = wc_ValidateEccsiPair(NULL, WC_SHA256, id, 1, ssk, pvt, &valid);
    if (ret != BAD_FUNC_ARG)
        return -10043;
    /* No key set */
    ret = wc_ValidateEccsiPair(&key, WC_SHA256, id, 1, ssk, pvt, &valid);
    if (ret != BAD_STATE_E)
        return -10044;
    ...
    ret = wc_EncodeEccsiPvt(NULL, NULL, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10058;
    ret = wc_EncodeEccsiPvt(&key, pvt, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10059;
    ret = wc_EncodeEccsiPvt(&key, NULL, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10060;
    ret = wc_EncodeEccsiPvt(NULL, pvt, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10061;
    ...
    ret = wc_ExportEccsiPublicKey(NULL, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10078;
    ret = wc_ExportEccsiPublicKey(&key, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10079;
    ret = wc_ExportEccsiPublicKey(NULL, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10080;
    /* No key to export */
    ret = wc_ExportEccsiPublicKey(&key, data, &sz);
    if (ret != BAD_STATE_E)
        return -10081;
    ...
    ret = wc_HashEccsiId(NULL, WC_SHA256, NULL, 1, NULL, NULL, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10085;
    ret = wc_HashEccsiId(&key, WC_SHA256, id, 1, pvt, hash, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10086;
    ret = wc_HashEccsiId(&key, WC_SHA256, id, 1, pvt, NULL, &hashSz);
    if (ret != BAD_FUNC_ARG)
        return -10087;
    ret = wc_HashEccsiId(&key, WC_SHA256, id, 1, NULL, hash, &hashSz);
    if (ret != BAD_FUNC_ARG)
        return -10088;
    ret = wc_HashEccsiId(&key, WC_SHA256, NULL, 1, pvt, hash, &hashSz);
    if (ret != BAD_FUNC_ARG)
        return -10089;
    ret = wc_HashEccsiId(NULL, WC_SHA256, id, 1, pvt, hash, &hashSz);
    if (ret != BAD_FUNC_ARG)
        return -10090;
    ret = wc_HashEccsiId(&key, WC_SHA256, id, 1, pvt, hash, &hashSz);
    if (ret != BAD_STATE_E)
        return -10091;
...
static int eccsi_kat_verify_test(EccsiKey* key, ecc_point* pvt)
{
    ...
    ret = wc_DecodeEccsiPvt(key, pvtData, sizeof(pvtData), pvt);
    if (ret != 0)
        return -10109;

    ret = wc_ValidateEccsiPvt(key, pvt, &valid);
    if (ret != 0)
        return -10110;
    if (!valid)
        return -10111;

    ret = wc_HashEccsiId(key, WC_SHA256, id, idSz, pvt, hash, &hashSz);
    if (ret != 0)
        return -10112;
...
static int eccsi_enc_dec_pair_test(EccsiKey* priv, mp_int* ssk, ecc_point* pvt)
{
    ...
    ret = wc_EncodeEccsiPvt(priv, pvt, NULL, &sz);
    if (ret != LENGTH_ONLY_E)
        return -10126;
    if (sz != 32 * 2)
        return -10127;

    ret = wc_EncodeEccsiPvt(priv, pvt, data, &sz);
    if (ret != 0)
        return -10128;
    if (sz != 32 * 2)
        return -10129;
...
static int eccsi_imp_exp_pubkey_test(EccsiKey* key1, EccsiKey* key2)
{
    ...
    ret = wc_ExportEccsiPublicKey(key1, NULL, &sz);
    if (ret != LENGTH_ONLY_E)
        return -10136;
    if (sz != 32 * 2)
        return -10137;

    ret = wc_ExportEccsiPublicKey(key1, data, &sz);
    if (ret != 0)
        return -10138;
    ...
    sz = sizeof(pubData);
    ret = wc_ExportEccsiPublicKey(key2, pubData, &sz);
    if (ret != 0)
        return -10140;
                               ...
                               mp_int* ssk, ecc_point* pvt)
{
    ...
    ret = wc_MakeEccsiPair(priv, rng, WC_SHA256, id, idSz, ssk, pvt);
    if (ret != 0)
        return -10144;

    ret = wc_ValidateEccsiPair(pub, WC_SHA256, id, idSz, ssk, pvt, &valid);
    if (ret != 0)
        return -10145;
    if (!valid)
        return -10146;
                                  ...
                                  mp_int* ssk, ecc_point* pvt)
{
    ...
    ret = wc_HashEccsiId(priv, WC_SHA256, id, idSz, pvt, hashPriv, &hashSz);
    if (ret != 0)
        return -10147;
    if (hashSz != 32)
        return -10148;
    ret = wc_HashEccsiId(priv, WC_SHA256, id, idSz, pvt, hashPub, &hashSz);
    if (ret != 0)
        return -10149;
    if (hashSz != 32)
        return -10150;
...
int eccsi_test(void)
{
    ...
    ret = eccsi_api_test(&rng, &ssk, pvt);
    if (ret != 0)
        return ret;
...
#ifdef WOLFCRYPT_HAVE_SAKKE
static int sakke_api_test(WC_RNG* rng, ecc_point* rsk)
{
    int ret;
    SakkeKey key;
    byte id[1] = { 0x00 };
    ...
    sz = sizeof(data);
    ret = wc_EncodeSakkeRsk(NULL, NULL, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10233;
    ret = wc_EncodeSakkeRsk(&key, rsk, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10234;
    ret = wc_EncodeSakkeRsk(&key, NULL, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10235;
    ret = wc_EncodeSakkeRsk(NULL, rsk, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10236;
    ...
    ret = wc_ExportSakkePublicKey(NULL, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10250;
    ret = wc_ExportSakkePublicKey(&key, data, NULL);
    if (ret != BAD_FUNC_ARG)
        return -10251;
    ret = wc_ExportSakkePublicKey(NULL, data, &sz);
    if (ret != BAD_FUNC_ARG)
        return -10252;
...
static int sakke_kat_derive_test(SakkeKey* key, ecc_point* rsk)
{
    ...
    /* Make sure the key public key can is exportable - convert to Montgomery
     * form in Validation.
     */
    ret = wc_ExportSakkePublicKey(key, pubKey, &sz);
    if (ret != 0)
        return -10331;
    if (sz != sizeof(pubKey))
        return -10332;
    if (XMEMCMP(pubKey, pubData, sizeof(pubKey)) != 0)
        return -10333;
...
static int sakke_make_key_test(SakkeKey* priv, SakkeKey* pub, WC_RNG* rng,
    ecc_point* rsk)
{
    int ret;
    byte data[440];
    ...
    ret = wc_ExportSakkePublicKey(priv, NULL, &sz);
    if (ret != LENGTH_ONLY_E)
        return -10357;
    if (sz != 256)
        return -10358;
    sz--;
    ret = wc_ExportSakkePublicKey(priv, data, &sz);
    if (ret == 0)
        return -10359;
    sz++;
    ret = wc_ExportSakkePublicKey(priv, data, &sz);
    ...
    ret = wc_ExportSakkePublicKey(pub, pubData, &sz);
    if (ret != 0)
        return -10364;
    if (sz != 256)
        return -10365;
    ...
    sz = sizeof(data);
    ret = wc_EncodeSakkeRsk(priv, rsk, data, &sz);
    if (ret != 0)
        return -10372;
    if (sz != 256)
        return -10373;
                         ...
                         ecc_point* rsk)
{
    ...
    ret = wc_SetSakkePointI(pub, ssv, ssvSz, pointI, sz);
    if (ret != 0)
        return -10390;
...
int sakke_test(void)
{
    ...
    ret = sakke_make_key_test(&priv, &pub, &rng, rsk);
    if (ret != 0)
        return ret;
...
static int ed25519_test_cert(void)
{
    ...
    if (file == NULL) {
        ERROR_OUT(-10724, done);
    }
    bytes = XFREAD(tmp, 1, FOURK_BUF, file);
    XFCLOSE(file);
#else
    /* No certificate to use. */
    ERROR_OUT(-10725, done);
#endif
    ...
    if (file == NULL) {
        ERROR_OUT(-10727, done);
    }
    bytes = XFREAD(tmp, 1, FOURK_BUF, file);
    XFCLOSE(file);
#else
    /* No certificate to use. */
    ERROR_OUT(-10728, done);
#endif
...
static int ed448_test_cert(void)
{
    ...
    if (file == NULL) {
        ERROR_OUT(-11424, done);
    }
    bytes = XFREAD(tmp, 1, FOURK_BUF, file);
    XFCLOSE(file);
#else
    /* No certificate to use. */
    ERROR_OUT(-11425, done);
#endif
    ...
    if (file == NULL) {
        ERROR_OUT(-11427, done);
    }
    bytes = XFREAD(tmp, 1, FOURK_BUF, file);
    XFCLOSE(file);
#else
    /* No certificate to use. */
    ERROR_OUT(-11428, done);
#endif
...
int blob_test(void)
{
    ...
    ret = wc_caamCreateBlob((byte*)iv, sizeof(iv), blob, &outSz);
    if (ret != 0) {
        ERROR_OUT(-13302, exit_blob);
    }

    ret = wc_caamOpenBlob(blob, outSz, out, &outSz);
    if (ret != 0) {
        ERROR_OUT(-13303, exit_blob);
    }

    if (XMEMCMP(out, iv, sizeof(iv))) {
        ERROR_OUT(-13304, exit_blob);
    }
    ...
    if (ret != 0) {
        ERROR_OUT(-13305, exit_blob);
    }

    ret = wc_caamOpenBlob(blob, outSz, out, &outSz);
    if (ret != 0) {
        ERROR_OUT(-13306, exit_blob);
    }

    if (XMEMCMP(out, text, sizeof(text))) {
        ERROR_OUT(-13307, exit_blob);
    }
