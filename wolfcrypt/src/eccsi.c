 ...
 * @param  [in]      key   ECCSI key.
 * @param  [out]     data  Buffer to hold encoded ECCSI key.
 * @param                  NULL when requesting required length.
...
int wc_ExportEccsiPublicKey(EccsiKey* key, byte* data, word32* sz)
{
    ...
    if (err == 0) {
        ...
        else if (*sz < (word32)key->ecc.dp->size * 2) {
            err = BUFFER_E;
        }
        else {
            *sz = key->ecc.dp->size * 2;
 ...
 *
 * @param  [in]   key   ECCSI key.
 * @param  [in]   data  Buffer to encode key pair into.
 * @param  [in]   sz    Size of encoded pair data in bytes.
 * @param  [out]  ssk   Secret Signing Key as an MP integer.
 * @param  [out]  pvt   Public Validation Token (PVT) as an ECC point.
 * @return  0 on success.
...
/*
 * Scalar multiply the base point of the curve.
 *
 * @param  [in]   key   ECCSI key.
        ...
        int map)
{
    ...
    if ((key->ecc.idx != ECC_CUSTOM_IDX) &&
        ...
        err = sp_ecc_mulmod_base_256(n, res, map, key->heap);
    }
    else {
        err = NOT_COMPILED_IN;
    }
 ...
 * Use wc_HashEccsiId() to calculate the hash.
 *
 * @param  [in]   key       ECCSI key.
 * @param  [in]   rng       Random number generator.
 * @param  [in]   hashType  Type of hash algorithm. e.g. WC_SHA256
