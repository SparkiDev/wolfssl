...
/**
 * Export the SAKKE key as DER encoded public/private ECC key.
 *
 ...
 * @return  BUFFER_E when size of buffer is too small.
 */
int wc_ExportSakkePublicKey(SakkeKey* key, byte* data, word32* sz)
{
    int err = 0;
 ...
 * @param  [out]     data  Buffer to hold encoded RSK.
 * @param  [in,out]  sz    In, size of buffer in bytes.
...
int wc_EncodeSakkeRsk(SakkeKey* key, ecc_point* rsk, byte* out, word32* sz)
{
    ...
    if (err == 0) {
        /* Write out the public key point's x ordinate into key size bytes. */
        err = mp_to_unsigned_bin_len(rsk->x, out, key->ecc.dp->size);
    }
    if (err == 0) {
        out += key->ecc.dp->size;
        /* Write out the public key point's y ordinate into key size bytes. */
        err = mp_to_unsigned_bin_len(rsk->y, out, key->ecc.dp->size);
    }
    if (err == 0) {
        *sz = key->ecc.dp->size * 2;
    }
        ...
        ecc_point* rsk)
{
    ...
    if (err == 0) {
        /* Read the public key point's x value from key size bytes. */
        err = mp_read_unsigned_bin(rsk->x, data, key->ecc.dp->size);
    }
    if (err == 0) {
        data += key->ecc.dp->size;
        /* Read the public key point's y value from key size bytes. */
        err = mp_read_unsigned_bin(rsk->y, data, key->ecc.dp->size);
    }
    if (err == 0) {
        err = mp_set(rsk->z, 1);
    }
        ...
        ecc_point* z, ecc_point* i)
{
    ...
    /* [b]P */
    if (err == 0) {
        err = sakke_mulmod_base(key, b, i, 0);
    }
    /* [b]P + Z_S */
    if (err == 0) {
        err = mp_montgomery_setup(&key->prime, &mp);
...
int wc_SetSakkePointITable(SakkeKey* key, byte* table, word32 len)
{
    ...
    if ((err == 0) && (key->ecc.dp->id == ECC_SAKKE_1) && (len != 0) &&
            (len != 64*1024)) {
        err = BUFFER_E;
    }
 ...
 * @param  [in]  idSz  Size of identifier in bytes.
 * @param  [in]  z     ECC point - KMS Public Key Z_S.
 * @param  [in]  r     MP integer - hash of SSV and b.
